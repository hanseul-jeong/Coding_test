def solution(relation):
    ...
    row_n = len(relation)
    cand_col = [i for i in range(0, col_n)]
    import numpy as np
    relation = np.array(relation)
    for i in range(1, col_n):
        if len(cand_col) < i:
            break
        combs = list(itertools.combinations(cand_col, i))
        del_indice = []
        for c in combs:
            inf = set([tuple(content) for content in relation[:,c]])
            # print(inf)
            if len(inf) == row_n:
                answer += 1
                del_indice.extend([idx for idx in c])            
        for idx in reversed(list(set(del_indice))):
            del cand_col[idx]
