...
castle, maps = [], []
rounds = [((ix//3)-1,(ix%3)-1) for ix in range(9) if ix != 4 ]
for i in range(X):
    castle.append([int(s) if s.isdigit() else s for s in sys.stdin.readline().split()[0]])
    maps.append([0]*Y)
...
for x in range(1, X):
    for y in range(1, Y):
        if castle[x][y] != '.':
            n_blanks = 0
            for dx, dy in rounds:  # edges
                n_blanks += 1 if castle[x+dx][y+dy] == '.' else 0
            maps[x][y] = n_blanks
n_waves, changes = -1, [(0,0)]
while changes:
    n_waves += 1
    # deform castle
    for x, y in changes:
        if not x and not y: continue
        castle[x][y], maps[x][y] = '.', 0
        # grow blanks
        for dx, dy in rounds:
            if castle[x+dx][y+dy] != '.':  maps[x+dx][y+dy] += 1
    # find changes
    changes = []
    for x in range(1, X):
        for y in range(1, Y):
            if not maps[x][y]: continue
            if castle[x][y] <= maps[x][y]:
                changes.append((x, y))
